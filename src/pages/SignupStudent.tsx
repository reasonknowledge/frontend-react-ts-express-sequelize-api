...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("Erreur dans le catch:", err);
      console.error("Type d'erreur:", err instanceof TypeError ? "TypeError" : typeof err);
      
      if (err instanceof TypeError && err.message.includes("Failed to fetch")) {
        alert("Impossible de se connecter au serveur. Vérifiez que le backend est démarré sur http://localhost:3000");
      } else {
    ...
    <div className="flex flex-col md:flex-row justify-center items-center min-h-screen bg-gray-50 p-4">
      <div className="flex flex-col md:flex-row justify-center items-center gap-8 max-w-6xl">
        ...
        >
          ...
          <div>
            <Input name="email" autoComplete="true" type="email" placeholder="Email" className="w-full focus-bootstrap-shadow" />
          ...
          <Button
            ...
            className="bg-blue-200 text-white hover:bg-blue-300 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
          >
